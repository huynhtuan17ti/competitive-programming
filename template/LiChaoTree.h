...
template <typename data_t> struct Line {
  data_t a, b;

  Line() : a(0), b(INF) {}
  Line(data_t a, data_t b) : a(a), b(b) {}

  data_t Eval(data_t x) { return a * actual_x[x] + b; }
};

template <typename data_t> struct Node {
...
public:
  ...
  T Query(T x) { return Query(root_, xmin_, xmax_, x); }

  ~LiChaoTree() { delete root_; }
...
private:
  ...
  void InsertLine(NodeType *&node, T L, T R, LineType x) {
    if (node == nullptr)
      node = new NodeType();
    if (node->line.Eval(L) > x.Eval(L))
      swap(node->line, x);
    if (node->line.Eval(R) <= x.Eval(R))
      return;
    if (L == R)
      return;
    T mid = (L + R) >> 1;
